...
int main()
{
  ...
  auto trueBB = BasicBlock::create(module, "trueBB_while", climbStairsFun);      // true分支
  auto falseBB = BasicBlock::create(module, "falseBB_while", climbStairsFun);    // false分支
  builder->create_br(condBB);

  builder->set_insert_point(condBB);
  auto aload = builder->create_load(a);
  auto icmp = builder->create_icmp_lt(aload, CONST_INT(0));   
